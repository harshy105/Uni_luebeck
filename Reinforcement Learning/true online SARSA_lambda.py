...
def replacing_trace(trace, activeTiles, lam, gamma, alpha):
    ...
    # Todo : update the trace here.
    trace = gamma*lam*trace
    trace[activeTiles] = (1-alpha)*trace[activeTiles] + 1
    return trace
...
class SARSA_lambda_Learner(object):
    def __init__(self, env, traceUpdate):
        ...
        self.Q_prev = self.Q.copy() # next Q value
        self.Q_next = self.Q.copy() # previous Q value
        self.I = np.zeros((self.obs_bins[0]+1, self.obs_bins[1]+1, self.action_shape)) #IsS_t as per the eqution 16 in the submission
    ...
    def learn(self, obs, action, reward, done, next_obs, next_action):
        ...
        td_delta = reward + self.gamma *(not done)* self.Q[discretized_next_sa] - self.Q[discretized_sa]
        ...
        #update the IsS_t
        self.I = 0*self.I
        self.I[discretized_sa] = 1  
        # To Do: update the q table        
        self.Q_next = self.Q + self.alpha*(td_delta+self.Q[discretized_sa]-self.Q_prev[discretized_sa])*self.trace - self.alpha*self.I*(self.Q[discretized_sa]-self.Q_prev[discretized_sa])
        self.Q_prev = self.Q
        self.Q = self.Q_next       
...
def train(agent, env, MAX_NUM_EPISODES):
    ...
    # Todo: Return the trained policy as an 2D-array
...
if __name__ == "__main__":
    ...
    np.save('D:/Robotics-Lübeck/RL/RL Assignment 3/true_sarsa_lambda_3.npy',episodic_returns)