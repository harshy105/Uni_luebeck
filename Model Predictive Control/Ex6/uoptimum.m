function u_optimum = uoptimum(A,B,Gu,hu,Gx,hx,Gf,hf,P,Q,R,N,x0)
...
Gx_bar = kron(eye(N-1),Gx);
hx_bar = kron(ones(N-1,1),hx);
Gx_bar = [Gx_bar zeros(size(Gx_bar,1),size(Gf,2));
    zeros(size(Gf,1),size(Gx_bar,2)) Gf];
hx_bar = [hx_bar; hf];
